...
resource "aws_instance" "backend" {
  ...
  # This startup script runs when the instance is first launched.
  # It automates the entire setup process.
  user_data = <<-EOF
              #!/bin/bash
              # Basic system update
              yum update -y
              
              # Files will be uploaded by file provisioner
              # Docker installation moved to provisioner
              EOF
...
# Remove the git clone from user_data and add file provisioners instead
...
resource "null_resource" "upload_backend_files" {
  ...
  provisioner "file" {
    source      = "${path.module}/../Dockerfile.fastapi"
    destination = "/home/ec2-user/app/Dockerfile.fastapi"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ls -la",
      "docker --version",
      "docker build -f Dockerfile.fastapi -t movie-sentiment-backend .",
...
resource "null_resource" "upload_frontend_files" {
  ...
  provisioner "file" {
    source      = "${path.module}/../Dockerfile.streamlit"
    destination = "/home/ec2-user/app/Dockerfile.streamlit"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ls -la",
      "docker --version",
      "docker build -f Dockerfile.streamlit -t movie-sentiment-frontend .",
